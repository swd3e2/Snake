...
public:
	void initialize(ApplicationSettings* settings, SceneManager* sceneManager, Renderer* renderer, EventSystem* eventSystem)
	{
		//physicsSystem	= std::make_unique<PhysicsSystem>(sceneManager);
		renderSystem	= std::make_unique<RenderSystem>(sceneManager, settings, physicsSystem.get(), renderer, eventSystem);
		//scriptSystem	= std::make_unique<ScriptSystem>(sceneManager);
		//cameraSystem	= std::make_unique<CameraSystem>(sceneManager);
		//playerSystem	= std::make_unique<PlayerSystem>(sceneManager, eventSystem, physicsSystem.get());
